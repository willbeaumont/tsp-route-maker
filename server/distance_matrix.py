...
class DistanceMatrix:
    def __init__(self, addresses, is_distance_type):
        ...
        self.is_distance_type = (
            is_distance_type  # option false:travel time based matrix
        )
    ...
    def send_request(self, origin_addresses, dest_addresses):
        ...
        request = "https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&mode=walking"
    ...
    def build_matrix(self, response):
        ...
        for row_index, row in enumerate(response["rows"]):
            ...
            row_list = [
                row["elements"][j][matrix_type]["value"]
                for j in range(len(row["elements"]))
                if row["elements"][j]["status"] == "OK"
            ]
